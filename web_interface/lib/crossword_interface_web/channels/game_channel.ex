defmodule CrosswordInterfaceWeb.GameChannel do
  use CrosswordInterfaceWeb, :channel

  alias CrosswordEngine.{Game, GameSupervisor}
  alias CrosswordInterfaceWeb.Presence
  ...
  def handle_in("position_crossword", payload, socket) do
    %{"player" => player, "island" => island, "row" => row, "col" => col} = payload
    player = String.to_existing_atom(player)
    island = String.to_existing_atom(island)
    case Game.position_crossword(via(socket.topic), player, island, row, col) do
      :ok -> {:reply, :ok, socket}
      _ -> {:reply, :error, socket}
    end
  end

  def handle_in("set_crossword", player, socket) do
    player = String.to_existing_atom(player)
    case Game.set_crossword(via(socket.topic), player) do
      {:ok, board} ->
        broadcast! socket, "player_set_crossword", %{player: player}
