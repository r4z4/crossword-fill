defmodule CrosswordEngine.Game do
  use GenServer, start: {__MODULE__, :start_link, []}, restart: :transient, type: :worker

  alias CrosswordEngine.{Board, Guesses, Rules, Crossword, Rules, Coordinate}
  ...
  def handle_call({:position_crossword, player, key, row, col}, _from, state_data) do
    board = player_board(state_data, player)
    with {:ok, rules} <-
           Rules.check(state_data.rules, {:position_crossword, player}),
         {:ok, coordinate} <-
           Coordinate.new(row, col),
         {:ok, island} <-
            Crossword.new(key, coordinate),
         %{} = board <-
            Board.position_crossword(board, key, island)
    do
      state_data
      ...
      {:error, :invalid_crossword_type} ->
        {:reply, {:error, :invalid_crossword_type}, state_data}
      {:error, :overlapping_crossword} ->
        {:reply, {:error, :overlapping_crossword}, state_data}
    end
  end

  def handle_call({:set_crossword, player}, _from, state_data) do
    board = player_board(state_data, player)
    with {:ok, rules} <- Rules.check(state_data.rules, {:set_crossword, player}),
         true         <- Board.all_crossword_positioned?(board)
    do
      state_data
      |> update_rules(rules)
      ...
      :error -> {:reply, :error, state_data}
      false  -> {:reply, {:error, :not_all_crossword_positioned}, state_data}
    end
  end
  ...
  def handle_call({:guess_coordinate, player_key, row, col}, _from, state_data) do
    ...
    with {:ok, rules} <- Rules.check(state_data.rules, {:guess_coordinate, player_key}),
         ...
         {hit_or_miss, forested_crossword, win_status, opponent_board} <- Board.guess(opponent_board, coordinate),
    ...
    do
      ...
      |> update_rules(rules)
      |> reply_success({hit_or_miss, forested_crossword, win_status})
    else
      :error -> {:reply, :error, state_data}
  ...
  def position_crossword(game, player, key, row, col) when player in @players, do:
    GenServer.call(game, {:position_crossword, player, key, row, col})

  def set_crossword(game, player) when player in @players, do:
    GenServer.call(game, {:set_crossword, player})
